...
function showGrid($date) {
	...
	*/	}
		...
		if ($set['eventHBox']) {
			...
			if ($no_count_allocated_drivers>0){
				while($rowDrivers = mysqli_fetch_object($rDrivers))
				{
					...
					$popText .= '<br/>'.$rowDrivers->driver_name.' : '.$rowDrivers->driver_phone_no;
		...
		$strSQLDriver="SELECT events_drivers_id,drivername,driverbgcolor,driverid FROM [db]events_drivers WHERE eventid=".$evt['eid']." AND taskdate='".$date."' ORDER BY drivername";
			...
			switch($noOfDrivers)
			{
				...
				default:
					...
					while($rowDriver = mysqli_fetch_object($rSet))
					{
						...
						echo "<li class=\"drivercolorbox\" title=\"".$rowDriver->drivername."\" driver_li_id=\"".$rowDriver->driverid."\"  event_id=\"".$evt['eid']."\"  id=\"".$rowDriver->events_drivers_id."\" event_driver_id=\"".$rowDriver->events_drivers_id."\" drivercolour=\"".$rowDriver->driverbgcolor."\" style=\"width:".$intPercentage."%; background-color:".$rowDriver->driverbgcolor.";\">&nbsp;</li>";
