...
function retrieve($start,$end,$iFilter='',$xFilter='') {
	...
	"SELECT
		...
		u.color AS uco,
		t.group_name AS tit,
		t.no_of_visitors AS vsts,
		i.details as des,
		t.arrival_date AS arrday,
		t.dep_date AS depday,
		t.trip_id AS tripid
	FROM [db]events e
	INNER JOIN [db]categories c ON c.category_id = e.category_id
	INNER JOIN [db]users u ON u.user_id = e.user_id
	INNER JOIN [db] tbl_itinerary i ON i.itinerary_id=e.itinerary_id
	INNER JOIN [db]tbl_trips t ON 	t.trip_id=i.trip_id 
	WHERE t.deleted=0 AND i.deleted=0 AND e.status >= 0".$filter;
//label represents Vehicle details and the label 2 represents phone nummber
	...
	ORDER BY
		t.arrival_date ASC limit 2500";
	$rSet = dbQuery($q1);
	
	
	
//echo "NR: ".mysqli_num_rows($rSet)." "; //TEST
	while ($row = mysqli_fetch_assoc($rSet)) {
		if (!$row['pri'] or ($row['uid'] == $userId) or $admin) { //private: only for current user + admin
...
function processEvent($from, $till, $eStart, $eEnd, &$row) {
	...
	for($i=$sTs;$i<=$eTs;$i+=86400) { //increment 1 day
		...
		if (strpos($row['xda'], $curD) === false) { //no exceptions
			...
			$evt['arrday'] = $row['arrday'];// arrival day
			$evt['depday'] = $row['depday'];// departure day
			$evt['tripid'] = $row['tripid'];// to be used to mark coming in group
