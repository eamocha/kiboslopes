...
function createDiv(divId, x, y) {;
	...
	var xpDiv = getDateElement(divId);
...
//vars dFormat, tFormat, wStart, dpMonths, dpWkdays, dpClear and dpToday must be defined outside dtpicker.js !

function dPicker(cB,form,dateFieldId1,dateFieldId2) {

	var dateField1 = getDateElement(dateFieldId1);
	var dateField2 = (dateFieldId2) ? getDateElement(dateFieldId2) : "";
...
function refreshDP(year, month, day) { //display/refresh date picker
  ...
  html += "</td>\n</tr>\n</table>\n";
 	getDateElement("dpDiv").innerHTML = html;
}
...
function updateDateField(yyyymmdd) {
	var dateField = getDateElement(dateFieldId);
	var dpDiv = getDateElement("dpDiv");
	if (yyyymmdd) {
...
function tPicker(timeFieldId) {
	var timeField = getDateElement(timeFieldId);
	var hhmm;
...
function updateTimeField(timeFieldId, timeString) {
	...
	var tpDiv = getDateElement("tpDiv");
