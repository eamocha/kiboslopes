...
if($trip_id!=0){
	...
	$strSQLAllEvents = "SELECT e.event_id,e.s_date,e.itinerary_id FROM `events` AS e WHERE e.s_date>='$eventdate' AND itinerary_id IN (SELECT itinerary_id FROM tbl_itinerary WHERE trip_id=$trip_id AND deleted=0) ORDER BY e.s_date";
