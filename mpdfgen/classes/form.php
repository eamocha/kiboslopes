...
function print_ob_text($objattr,$w,$h,$texto,$rtlalign,$k,$blockdir): void {
		// TEXT/PASSWORD INPUT
...
function print_ob_textarea($objattr,$w,$h,$texto,$rtlalign,$k,$blockdir): void {
		   // TEXTAREA
		    if ($this->mpdf->useActiveForms) {
				// Flags: 1 - Readonly; 2 - Required; 3 - No export; 13 - textarea; 14 - Password
				$flags = array();
...
function print_ob_select($objattr,$w,$h,$texto,$rtlalign,$k,$blockdir): void {
		// SELECT
			if ($this->mpdf->useActiveForms) {
...
function print_ob_imageinput($objattr,$w,$h,$texto,$rtlalign,$k,$blockdir): void {
...
function print_ob_button($objattr,$w,$h,$texto,$rtlalign,$k,$blockdir): void {
		// BUTTON
		     if ($this->mpdf->useActiveForms) {
				// Flags: 1 - Readonly; 3 - No export; 
				$flags = array();
...
function print_ob_checkbox($objattr,$w,$h,$texto,$rtlalign,$k,$blockdir,$x,$y): void {
		// CHECKBOX
		     if ($this->mpdf->useActiveForms) {
...
function print_ob_radio($objattr,$w,$h,$texto,$rtlalign,$k,$blockdir,$x,$y): void {
		// RADIO
			if ($this->mpdf->useActiveForms) {
				// Flags: 1 - Readonly; 2 - Required; 3 - No export; 
				$flags = array();
...
// In _putpages
function countPageForms($n, &$totaladdnum): void {
...
// In _putpages
function addFormIds($n, &$s, &$annotid): void {
				foreach( $this->forms as $form ) {
					if ( $form['page'] == $n ) {
						$s .= ($annotid) . ' 0 R ';
						$annotid++;
...
// In _putannots
function _putFormItems($n, $hPt): void {
...
// In _putannots
function _putRadioItems($n): void {
		// Output Radio Groups
		$key = 1;
...
function _putFormsCatalog(): void {
...
function SetFormButtonJS( $name, $js ): void {
...
function SetFormChoiceJS( $name, $js ): void {
	$js = str_replace("\t",' ', trim($js) );
	if ( isset($name) && isset($js) ) {
...
function SetFormTextJS( $name, $js): void {
...
function SetFormText( $w, $h, $name, $value = '', $default = '', $title = '', $flags = array(), $align='L', $hidden = false, $maxlen=-1, $js='', $background_col=false, $border_col=false ): void {
	// Flags: 1 - Readonly; 2 - Required; 3 - No export; 13 - textarea; 14 - Password
	$this->formn++;
	if( $align == 'C' ) { $align = '1'; }
	...
	function SetFormChoice( $w, $h, $name, $flags, $array, $align='L', $js = '' ): void {
		$this->formn++;
	...
	// CHECKBOX
	function SetCheckBox( $w, $h, $name, $value, $title = '', $checked = false, $flags = array(), $disabled=false  ): void {
	...
	// RADIO
	function SetRadio( $w, $h, $name, $value, $title = '', $checked = false, $flags = array(), $disabled=false ): void {
		$this->SetFormButton( $w, $h, $name, $value, 'radio', $title, $flags, $checked, $disabled );
		$this->mpdf->x += $w;
	}


	function SetFormReset( $w, $h, $name, $value = 'Reset', $title = '', $flags = array(), $background_col=false, $border_col=false, $noprint=false ): void {
		if (!$name) { $name = 'Reset'; }
	...
	function SetJSButton( $w, $h, $name, $value, $js, $image_id = 0, $title = '', $flags = array(), $indexed=false , $background_col=false, $border_col=false, $noprint=false ): void {
	...
	function SetFormSubmit( $w, $h, $name, $value = 'Submit', $url, $title = '', $typ = 'html', $method = 'POST', $flags = array(), $background_col=false, $border_col=false, $noprint=false): void {
		if (!$name) { $name = 'Submit'; }
	...
	function SetFormButtonText( $ca, $rc = '', $ac = '' ): void {
		if ($this->mpdf->onlyCoreFonts) { 
	...
	function SetFormButton( $bb, $hh, $name, $value, $type, $title = '', $flags = array(), $checked=false, $disabled=false, $background_col=false, $border_col=false, $noprint=false ): void {
		$this->formn++;
		if (!preg_match('/^[a-zA-Z0-9_:\-]+$/', $name)) {
			$this->mpdf->Error("Field [".$name."] must have a name attribute, which can only contain letters, numbers, colon(:), undersore(_) or hyphen(-)"); 
		}
...
function SetFormBorderWidth ( $string ): void {
	switch( $string ) {
...
function SetFormBorderStyle ( $string ): void {
	switch( $string ) { 
...
function SetFormBorderColor ( $r, $g=-1, $b=-1 ): void {
...
function SetFormBackgroundColor ( $r, $g=-1, $b=-1 ): void {
	if ( ($r==0 and $g==0 and $b==0) || $g==-1 )
...
function SetFormD ( $W, $S, $BC, $BG ): void {
...
function _put_button_icon( $array , $w, $h ): void {
