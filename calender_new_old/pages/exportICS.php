	...
	foreach ($evtList as $evtListDate) {
		foreach ($evtListDate as $evt) {
			if (!in_array($evt['eid'], $eidDone)) { //event not yet processed
				...
				if ($evt['ada']) {
					$icsBody .= "CREATED:".gmdate('Ymd\THis\Z', mktime(12,0,0,substr($evt['ada'],5,2),substr($evt['ada'],8,2)+1,substr($evt['ada'],0,4)))."\r\n";
				}
				if ($evt['mda']) {
					$icsBody .= "LAST-MODIFIED:".gmdate('Ymd\THis\Z', mktime(12,0,0,substr($evt['mda'],5,2),substr($evt['mda'],8,2)+1,substr($evt['mda'],0,4)))."\r\n";
				}
