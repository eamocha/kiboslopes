...
function addUser() { //add user account
	global $uid, $privAdmin, $privPost, $editUser, $uiLanguage, $ax, $uname, $email, $bgrnd, $rxEmail;
	...
	do {
		...
		$rSet = dbQuery("SELECT user_name FROM [db]users WHERE user_name = '".mysqli_real_escape_string($conn,$uname)."' AND status >= 0");
		if (mysqli_num_rows($rSet) > 0) { $msg = $ax['usr_uname_exists']; break; }
		$rSet = dbQuery("SELECT email FROM [db]users WHERE email = '".mysqli_real_escape_string($conn,$email)."' AND status >= 0");
...
function updateUser() { //update user account
	...
	do {
		...
		$result = dbQuery("UPDATE [db]users SET user_name = '".mysqli_real_escape_string($conn,$uname)."', email = '".mysqli_real_escape_string($conn,$email)."', sedit = $privAdmin, privs = $privPost, language = '$uiLanguage', color = '$bgrnd' WHERE user_id = $uid");
