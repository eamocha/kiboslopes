...
function addEvents() {
	...
	for ($i = 0; $i < $nofEvents; $i++) {
		if (empty($_POST['delete'][$i]) and $_POST['r_t'][$i] >= 0) { //if delete not ticked and repeat-type is valid
			$title = mysqli_real_escape_string($conn,htmlspecialchars_decode(trim($_POST['title'][$i])));
			$venue = mysqli_real_escape_string($conn,htmlspecialchars_decode(trim($_POST['venue'][$i])));
