...
try
{
  ...
  $sql="UPDATE tbl_itinerary SET `itenerary_type_id`=?,`date` = ?,`title` = ?,`details` = ?,`trip_id` = ?,`hotel_id` = ?,`deleted` = ?,`remarks` = ?,singles = ?,`doubles` = ?,`twins` = ?,`triples` = ?,`child_beds` =?,`family_rooms` =? 
  		   WHERE itinerary_id = ?"; 
  /* Prepare statement */
		...
		$stmt->bind_param('isssiiisiiiiiii',$itenerary_type_id,$indate,$day,$itnedetails,$tripid,$hotel,$deleted,$spreqs,$singles,$doubles,$twins,$triples,$child_beds,$family_rooms,$itnid);
		...
		if(isset($_REQUEST['include_hotel']))
		{
			...
			{
				$stmt->bind_param('iiissssdddddddddddddssisssi',$trip_hotel_id,$tripid,$hotel,$mode_of_payment,$currency,$kibo_reference,$hotel_reference,$singlesrate,$singlestotalcost,$dbrate,$dbtotalcost,$twinsrate,$twinstotalcost,$triplesrate,$triplestotalcost,$children_bedsrates,$chidrentotalcost,$family_roomrates,$familytotalcost,$extracost,$extracostcomments,$comments,$is_deleted,$payment_due,$currentdatetime,$last_modified_by,$hotel_charges_id);
			}
		...
		//DELETE the countries and add them again for consistency
		$stmt = $conn->prepare("DELETE FROM tbl_itinerary_country WHERE itinerary_id = ?");
		...
		$stmt = $conn->prepare("DELETE FROM tbl_visitor_itenerary WHERE itenerary_id = ?");
