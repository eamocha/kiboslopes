...
<li><a href="imprests_staff.php">Staff Imprests</a></li>
...
<tr id="search_form" height="45px" ><td colspan="3"><form id="formID" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="get"><label>Date</label><input type="text" class="text-input" id="dat" name="dat"  value="<?php echo $dat?>"? /> 
...
<tr class="table_headers">
  ...
  <td width="21%">Driver(s)  &amp;  Vehicle</td>

  <td width="7%">Visitors</td>
...
<?php $dbQuey=mysqli_query($conn,"SELECT e.remarks AS r,e.event_id as eid,c.category_id as d,c.label1 AS veh,c.name as dr,t.group_name AS trp, t.no_of_visitors as vsts, i.details as itn FROM events e
	...
	for($i=0; $i<$n;$i++){
        ...
        <tr class="alt_row1"><td><?php echo $t?></td><td><?php echo $itn?></td>
          <td><?php 
			  
			  $strCalenderDrivers = getDriversVehicles($eid, $dat, true);
		  ...
		  ?></td>
          <td align="center"><?php echo $vsts?></td><td><?php if($rmks=="") echo '-'; else echo $rmks?></td>
	...
	>Edit</a></td>
		...
		else{?> <tr class="alt_row2"><td><?php echo $t?></td><td><?php echo $itn?></td>
		  <td><?php 
