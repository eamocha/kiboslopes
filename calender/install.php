...
<body>
<header>
<h4>Operations  Event Calendar</h4>
</header>
...
if (isset($_POST['install'])) {
	do {
		...
		$connect = mysqli_select_db($link,$_POST['dbName']);
		if (!$connect) {
		...
		$rSet1 = mysqli_query($link,"SELECT count(*) FROM ".$_POST['dbPfix']."events");
		$rSet2 = mysqli_query($link,"SELECT count(*) FROM ".$_POST['dbPfix']."categories");
		$rSet3 = mysqli_query($link,"SELECT count(*) FROM ".$_POST['dbPfix']."users");
		$rSet4 = mysqli_query($link,"SELECT count(*) FROM ".$_POST['dbPfix']."settings");
		...
		$users = mysqli_query($link,"CREATE TABLE ".$_POST['dbPfix']."users (
			...
			login_0 DATE NOT NULL DEFAULT '2022-01-01',
			login_1 DATE NOT NULL DEFAULT '2022-01-01',
		...
		$categories = mysqli_query($link,"CREATE TABLE ".$_POST['dbPfix']."categories (
		...
		$events = mysqli_query($link,"CREATE TABLE ".$_POST['dbPfix']."events (
			...
			e_date DATE NOT NULL DEFAULT '2022-01-01',
			...
			r_until DATE NOT NULL DEFAULT '2022-01-01',
			notify TINYINT(1) NOT NULL DEFAULT '-1',
			not_mail VARCHAR(255) DEFAULT NULL,
			a_date DATE NOT NULL DEFAULT '2022-01-01',
			m_date DATE NOT NULL DEFAULT '2022-01-01',
			status TINYINT(1) NOT NULL DEFAULT '0',
			PRIMARY KEY (event_id)
			)")or die(mysqli_error($link));
		$settings = mysqli_query($link,"CREATE TABLE ".$_POST['dbPfix']."settings (
		...
		$category = mysqli_query($link,"INSERT INTO ".$_POST['dbPfix']."categories (category_id, name, sequence) VALUES (1,'no cat',1)");
		$public_user = mysqli_query($link,"INSERT INTO ".$_POST['dbPfix']."users (user_id, user_name, email, privs) VALUES (1,'Public Access',' ',1)");
		$admin_user = mysqli_query($link,"INSERT INTO ".$_POST['dbPfix']."users (user_id, user_name, email, password, sedit, privs) VALUES (NULL,'".$_POST['adminUname']."','".$_POST['adminEmail']."','".$crypt_pw."',1,3)");
		...
		if (file_put_contents('./lcaldbc.dat',$dbCrHdr.$dbCreds) === false) {
			...
			echo "<p>Unable to write the lcaldbc.dat file to the calendar root directory, please go <button type=\"button\" onclick=\"history.back()\">back</button> and check the permissions of the calendar's root directory (should be 755).</p>\n";
			break;
		}
//		chmod('./lcaldbc.dat',0600); //invisible to visitors
		...
		$settings = mysqli_query($link,"INSERT INTO ".$_POST['dbPfix']."settings VALUES
			('calendarTitle','".mysqli_real_escape_string($link,stripslashes($_POST['title']))."','Calendar title displayed in the top bar'),
			('calendarUrl','".mysqli_real_escape_string($link,stripslashes($_POST['url']))."','Calendar location (URL)'),
			('calendarEmail','".mysqli_real_escape_string($link,stripslashes($_POST['adminEmail']))."','Sender in and receiver of email notifications'),
			...
			('timeFormat','h:m','Time format (h:hh, m:mm, a:am|pm, A:AM|PM)'),
		...
		echo "<li>The calender version number and the (encrypted) database credentials have <br>been saved in the file <kbd>lcaldbc.dat</kbd> in the calendar root directory</li>\n";
		echo "</ol>\n";
		echo "<br>";
...
<p>Please enter the following data to install the Event Calendar:</p>
<br>
<div class="floatL">
...
<tr><td>Calendar Title:</td><td><input type="text" name="title" value="Kiboslopes operations Calendar"/></td></tr>
...
<footer><span class="floatR"> powered by <span class="footLB">Bluerange </span><span class="footLR">Kenya</span></span></footer>
</body>
</html>
