    ...
    */
    ...
    class FileData extends databaseData
    {
        ...
        function exists()
        {
            ...
            $result = mysqli_query($conn,$query, $this->connection);
	...
	This function load up all the fields in data table.*/
              ...
              SET
               ...
               WHERE
                    id = $this->id
                            ";
                    //echo $query;exit;
                             mysqli_query($conn,$query) or die('Error during updateData: ' . mysqli_error($conn)());
        }
        //return filesize
        ...
        function getUserIds($right)
        {
            ...
            $owner_result = mysqli_query($conn,$owner_query, $this->connection) or die("Error in query: ".$owner_query . mysqli_error($conn)() );
            $u_result = mysqli_query($conn,$u_query, $this->connection) or die("Error in query: " .$u_query . mysqli_error($conn)() );
            // result of $non_prev_user_query query.  Look above for more information.
            $non_prev_u_reslt = mysqli_query($conn,$non_prev_user_query, $this->connection) or die("Error in query: " .$non_prev_user_query . mysqli_error($conn)() );
	  	...
	  	FROM {$GLOBALS['CONFIG']['db_prefix']}$this->TABLE_DEPT_PERMS, {$GLOBALS['CONFIG']['db_prefix']}$this->TABLE_USER WHERE fid = $this->id AND 
            ...
            $d_result = mysqli_query($conn,$d_query, $this->connection) or die("Error in query: " .$d_query . mysqli_error($conn)() );
        ...
        {
            mysqli_query($conn,"UPDATE {$GLOBALS['CONFIG']['db_prefix']}$this->TABLE_DATA set status=$value where id = $this->id") or die(mysqli_error($conn)());
        }
	...
	getUserIds($right).*/
        ...
        function getReviewerID()
        {
            ...
            $result = mysqli_query($conn,$query, $this->connection) or die("Error in query: $query" . mysqli_error($conn)());
            $num_hits = mysqli_num_rows($result);
            if($num_hits != 1)
            {
        ...
        function setReviewerComments($comments)
        {
            ...
            $result = mysqli_query($conn,$query, $this->connection) or
