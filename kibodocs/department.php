...
elseif(isset($_REQUEST['submit']) && $_REQUEST['submit'] == 'delete')
{
    ...
    $reassign_list_result = mysqli_query($conn,$reassign_list_query, $GLOBALS['connection']) or die ("Error in query: $reassign_list_query. " . mysqli_error($conn)());
    ...
    if(mysqli_num_rows($reassign_list_result) < 1)
    {
...
elseif(isset($_REQUEST['deletedepartment']))
{
    ...
    $result = mysqli_query($conn,$query, $GLOBALS['connection']) or die ("Error when updating old department ID to re-assigned dept id: $query. " . mysqli_error($conn)());
    ...
    $result = mysqli_query($conn,$query, $GLOBALS['connection']) or die ("Error when updating user old department ID to re-assigned dept id: $query. " . mysqli_error($conn)());
    ...
    $result = mysqli_query($conn,$query, $GLOBALS['connection']) or die ("Error when updating dept_reviewer old department ID to re-assigned dept id: $query. " . mysqli_error($conn)());

    // Delete from department
    $query = "DELETE FROM {$GLOBALS['CONFIG']['db_prefix']}department where id='$_REQUEST[id]'";
    $result = mysqli_query($conn,$query, $GLOBALS['connection']) or die ("Error in deleting ID from department: $query. " . mysqli_error($conn)());
