...
if( !defined('User_class') )
{
    ...
    class User extends databaseData
    {
        ...
        function isAdmin()
        {
            ...
            $result = mysqli_query($conn,$query, $this->connection) or die("Error in querying: $query" . mysqli_error($conn)() );
            if(mysqli_num_rows($result) !=1 )
            {
                return false;
            }
        ...
        */
        function isReviewerForFile($file_id)
        {
            $query = "SELECT
                      ...
                      WHERE
                            ...
                            d.id = '$file_id'
                            ";                          
            $result = mysqli_query($conn,$query, $this->connection) or die("Error in query during isReviewerForFile call: " . mysqli_error($conn)());
        ...
        function getAllRevieweeIds() 
        {
            if($this->isAdmin())
            {
                ...
                $lresult = mysqli_query($conn,$lquery, $this->connection) or die("Error in query: $lquery" . mysqli_error($conn)());
                $lfile_data = array();
        ...
        function getRevieweeIds() 
        {
            if($this->isReviewer())
            {
                ...
                $num_depts = mysqli_num_rows($result);
        ...
        function getExpiredFileIds()
        {
            ...
            $lresult = mysqli_query($conn,$lquery) or die(mysqli_error($conn)());
            $llen = mysqli_num_rows($lresult);
            $file_data = array();
        ...
        function getNumExpiredFiles()
        {
            $lquery = "SELECT id FROM {$GLOBALS['CONFIG']['db_prefix']}data WHERE status=-1 AND owner = '$this->id'";
            $lresult = mysqli_query($conn,$lquery) or die(mysqli_error($conn)());
            return mysqli_num_rows($lresult);
        }
        ...
        function getCheckedOutFiles()
        {
            if ($this->isRoot())
            {
                ...
                $result = mysqli_query($conn,$query) or die("Error trying to create checked out files list: $query" . mysqli_error($conn)());
